#!/usr/bin/env python3
"""
Tool-calling evaluation — GLM variant using OpenAI Chat Completions API.

Differs from oss_eval_async_clean.py in exactly one way:
  LLM backend: OpenAI Responses API  →  OpenAI Chat Completions API
               (client.responses.create → client.chat.completions.create)

Use this for models served by vLLM that do not support the Responses API
(e.g. GLM-4.7).
"""
...
class WebSearchToolHandler:
    ...
    DAUM_API_URL = "https://daum-perplexity-search-adapter.toy.x.upstage.ai/search"
    ...
    def __init__(self, k: int = 5, use_jina: bool = True, include_get_document: bool = True,
        ...
        self.perplexity_api_key = get_api_key("PERPLEXITY_API_KEY") or os.environ.get("PERPLEXITY_API_KEY", "")
        self.daum_api_key = get_api_key("DAUM_SEARCH_API_KEY") or os.environ.get("DAUM_SEARCH_API_KEY", "")
        self.k = k
    ...
    def _search_daum(self, query: str) -> list:
        import requests
        resp = requests.post(
            self.DAUM_API_URL,
            headers={"Authorization": f"Bearer {self.daum_api_key}", "Content-Type": "application/json"},
            json={"query": query[:2000], "max_results": self.k, "reranking": "on"},
            timeout=30,
        )
        resp.raise_for_status()
        return [
            {"title": r.get("title", ""), "url": r.get("url", ""), "snippet": r.get("snippet", "")}
            for r in resp.json().get("results", [])[:self.k]
        ]

    def _search_perplexity(self, query: str) -> list:
        ...
        for attempt in range(max_retries):
            try:
                resp = requests.post(
                    ...
                    json={"model": "sonar", "messages": [{"role": "user", "content": query[:2000]}]},
                    timeout=60,
                )
            ...
            if resp.status_code == 429:
                ...
                print(f"[Perplexity] Rate limited, waiting {wait}s (attempt {attempt+1}/{max_retries})")
                time.sleep(wait)
                continue
            if not resp.ok:
                print(f"[Perplexity] HTTP {resp.status_code}, returning empty")
                return []
            data = resp.json()
            content = data.get("choices", [{}])[0].get("message", {}).get("content", "")
            citations = data.get("citations", [])
            # Normalise to title/url/snippet format: each citation becomes one result,
            # with the model's response text as a shared snippet.
            if citations:
                return [{"title": url, "url": url, "snippet": content[:500]} for url in citations[:self.k]]
            # No citations: return the response itself as a single result
            return [{"title": "Perplexity answer", "url": "", "snippet": content[:1000]}]
        print(f"[Perplexity] Rate limit persisted after {max_retries} retries, returning empty")
        return []

    def _run_search(self, query: str) -> list:
        if self.search_provider == "daum":
            return self._search_daum(query)
        if self.search_provider == "perplexity":
            return self._search_perplexity(query)
        return self._search_serper(query)

    def execute_tool(self, tool_name: str, arguments: dict) -> str:
        import requests  # noqa: F401 (used in sub-methods)
        if tool_name == "search":
            results = []
...
def make_tool_handler(search_client_type: str, args):
    ...
    if search_client_type == "async-web-search-access":
        provider = getattr(args, "search_provider", "serper")
        print(f"Tool handler: WebSearchToolHandler (provider={provider}, jina={args.use_jina})")
...
def run_conversation_with_tools(
    ...
    """Drive a Chat Completions tool-calling conversation to completion.
    ...
    for iteration in range(1, max_iterations + 1):
        try:
            response = client.chat.completions.create(**{**initial_request, "messages": messages})
        except Exception as e:
            print(f"  [iter {iteration}] API error: {e}", flush=True)
            break
        ...
        for tc in msg.tool_calls:
            try:
                result = tool_handler.execute_tool(tc.function.name, json.loads(tc.function.arguments))
...
def extract_answer(messages: list) -> str:
    """Extract the predicted answer from the final assistant message.
...
def process_single_item(
    ...
    if not getattr(args, "no_extra_body", False):
        request["extra_body"] = {
...
def parse_args():
    ...
    parser.add_argument("--search-provider", dest="search_provider", default="serper",
                        choices=["serper", "perplexity", "daum"],
                        help="Web search backend for async-web-search-access")
    parser.add_argument("--verbose", action="store_true")
    parser.add_argument("--no-extra-body", action="store_true",
                        help="Disable extra_body with chat_template_kwargs (for vLLM)")
