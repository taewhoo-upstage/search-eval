#!/bin/bash
#SBATCH --job-name=qwen3-sft-hermes-gaia
#SBATCH --partition=normal
#SBATCH --nodelist=Slurm-GPU-Node-[1-49]
#SBATCH --nodes=1
#SBATCH --gres=gpu:8
#SBATCH --mem=0
#SBATCH --output=/mnt/weka/post_training/pt2-search-agent/evaluation/orig_repo/ASearcher/logs/qwen3-sft-hermes-gaia-frames.log
#SBATCH --error=/mnt/weka/post_training/pt2-search-agent/evaluation/orig_repo/ASearcher/logs/qwen3-sft-hermes-gaia-frames.log
#
# Prerequisites:
...
# ─── Model configuration ────────────────────────────────────────────────────
MODEL_PATH="${MODEL_PATH:-/mnt/weka/post_training/checkpoints/Qwen3-30B-A3B-Base_sft/iter_0000499_hf}"
MODEL_NAME="${MODEL_NAME:-${MODEL_PATH##*/}}"
MODEL_URL=http://localhost:8000/v1

TEMPERATURE=0.6
TOP_P=0.95
MAX_GEN_TOKENS="${MAX_GEN_TOKENS:-16384}"
...
vllm serve "${MODEL_PATH}" \
  ...
  > "${LOG_DIR}/vllm_qwen3_sft_hermes.log" 2>&1 &
